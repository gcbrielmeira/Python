...
class Produto:
    ...
    def listar(mysql):
        ...
        resultados = cursor.fetchall() # executar
        cursor.close()
        # [Prouto(id, nome, preco)]
        # [ ] -> Array -> Lista
    ...
    def salvar(self, mysql):
        cursor = mysql.connection.cursor()
        cursor.execute("INSERT INTO produtos (nome, preco) VALUES (%5, %5)", (self.nome, self.preco))
    ...
    def atualizar(self, mysql):
        cursor = mysql.connection.cursor()
        cursor.execute("UPDATE produto SET nome = %s, preco=%s WHERE id = %s", (self.nome, self.preco, self.id))
        mysql.connection.commit()
        cursor.close()
